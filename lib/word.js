...
Word.prototype.add = function (row, cb) {
  ...
  let query_str =
    "insert into tbl_words (" + keys + ", example) values (" + values + ', "")';
...
Word.prototype.update = function (id, cb) {
  let self = this;
  self.conn.query(
    "update tbl_words set cnt=cnt+1 where id=?",
    [id],
...
Word.prototype.getFromApi = function (cb) {
  let self = this;
  const url =
    "http://dict-co.iciba.com/api/dictionary.php?key=" +
    config.iciba_key +
    "&type=json&w=" +
    encodeURIComponent(self.word);
  return request.get(
    {
      headers: {
        connection: false,
